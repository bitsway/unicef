...
function get_plan(from_month,from_year,to_month,to_year,access_type,access_type_val,ana_sector) {
		//alert(apipath+'get_plan_details?anaProgram='+anaProgram+'&anaProject='+anaProject+'&from_month='+from_month+'&from_year='+from_year+'&to_month='+to_month+'&to_year='+to_year+'&sector='+ana_sector);	
		
			$.ajax({				   
					url:apipath+'get_plan_details?access_type='+access_type+'&access_type_val='+access_type_val+'&from_month='+from_month+'&from_year='+from_year+'&to_month='+to_month+'&to_year='+to_year+'&sector='+ana_sector,
...
function get_achivement(from_month,from_year,to_month,to_year,access_type,access_type_val,ana_sector){
	...
	//alert(apipath+'get_achievement_details?anaProject='+anaProject+'&from_month='+from_month+'&from_year='+from_year+'&to_month='+to_month+'&to_year='+to_year+'&sector='+ana_sector);
	
	$.ajax({				   
		url:apipath+'get_achievement_details?access_type='+access_type+'&access_type_val='+access_type_val+'&from_month='+from_month+'&from_year='+from_year+'&to_month='+to_month+'&to_year='+to_year+'&sector='+ana_sector,
			  success: function(result) {
				  ach_list=result;
				  $('#ach_text').val(result);
				  compare_data()
...
function show_report(){
	...
	for (i=0;i<plan_ach_array_count;i++){		
		...
		if(plan_show_array[1]!=undefined){
			$('#plan_details_with_progress tbody').append('<tr><td>'+plan_show_array[0]+'</td><td>'+plan_show_array[1]+'</td><td>'+plan_show_array[2]+'</td><td>'+plan_show_array[3]+'</td><td>'+plan_show_array[4]+'</td><td>'+plan_show_array[5]+'</td><td>'+plan_show_array[6]+'</td><td>'+plan_show_array[7]+'</td></tr>');
